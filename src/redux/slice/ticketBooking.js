...
export const fetchTicketBookingDetails = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const params = { maLichChieu: '47557' };
...
const ticketBookingSlice = createSlice({
  ...
  reducers: {
    ...
    chooseSeat(state, action) {
      ...
      const idx = selectedSeats.findIndex((selectedSeat) => selectedSeat.id === seat.id);
      if (idx !== -1) {
    ...
    builder.addCase(bookTicket.fulfilled, (state) => {
      ...
      if (!state.selectedSeats.length) {
        state.modal = {
          ...
          title: "Thông báo",
          children: [
            "Vui lòng chọn ít nhất một chỗ ngồi! Bạn có thể mua được tối đa 5 ghế.",
          ],
          path: "",
        };
      } else {
...
export const { chooseSeat, closeModal } = ticketBookingSlice.actions;
